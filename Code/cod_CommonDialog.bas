Attribute VB_Name = "cod_CommonDialog"
...
Public Function GetFiles(Optional ByVal sTitle As String = "Open files...") As Variant
...
ProcExit:
  ...
  TempArray = Split(sFilenames, Chr(0))
  If UBound(TempArray) > 0 Then
    For ArrayItem = 1 To UBound(TempArray)
      TempArray(ArrayItem) = TempArray(0) & IIf(Right(TempArray(0), 1) = "\", "", "\") & TempArray(ArrayItem)
    Next
  ElseIf UBound(TempArray) = 0 Then
    OnlyOneFile = TempArray(0)
    ReDim TempArray(1)
    TempArray(1) = OnlyOneFile
  Else
    'nothing
  End If
  GetFiles = TempArray
